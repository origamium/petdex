...
Internal commands (invoked by tooling, not by hand): `petdex bubble <pre|post|stop>` is the hot-path hook runner agents call on tool events, and `petdex mcp-server` is the MCP server subprocess for MCP-capable agents (Codex, Claude Code, Cursor, Gemini, Junie, Antigravity, …). Set `PETDEX_MCP_AGENT` so bubbles and usage land under the right logo. Both bypass help/telemetry output on purpose.
